  ...
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    if (isLotting) {
      if (e.target.id === "lottery") {
        rotateObj.stop();
        btns.lottery.innerHTML = "Bắt đầu";
      } else {
        addQipao("Xổ số đang được rút thăm, rút thăm chậm hơn một chút ~~"); // addQipao("正在抽奖，抽慢一点点～～");
      }
      return false;
    }
    ...
    switch (target) {
      ...
      case "enter":
        ...
        addQipao(`Quay lại [${currentPrize.title}], đừng rời mắt.`);
        // rotate = !rotate;
        rotate = true;
        switchScreen("lottery");
        break;
      ...
      case "reset":
        let doREset = window.confirm(
          "Bạn có chắc chắn đặt lại dữ liệu không? Sau khi đặt lại, tất cả các giải thưởng đã quay sẽ bị xóa?"
          // "是否确认重置数据，重置后，当前已抽的奖项全部清空？"
        );
        if (!doREset) {
          return;
        }
        addQipao("Khởi tạo lại vòng quay, bắt đầu quay lại");
      ...
      case "lottery":
        ...
        addQipao(`Đang quay [${currentPrize.title}], cầu nguyện đi nào!`);
        // addQipao(`正在抽取[${currentPrize.title}],调整好姿势`);
        break;
      // 重新抽奖
      case "reLottery":
        if (currentLuckys.length === 0) {
          addQipao("Chưa có xổ số nên không bốc thăm lại được!!"); // addQipao(`当前还没有抽奖，无法重新抽取喔~~`);
          return;
        }
        setErrorData(currentLuckys);
        addQipao(`Quay lại [${currentPrize.title}],hãy chuẩn bị.`);
        setLotteryStatus(true);
...
function selectCard(duration = 600) {
  ...
  addQipao(
    ...
    `Chúc mừng [${text.join("、")}] đã nhận được [${
      currentPrize.title
    }], chúc mừng các bạn`
  );
...
function lottery() {
  ...
  //   return;
  // }
  btns.lottery.innerHTML = "Dừng quay";
  rotateBall().then(() => {
    // 将之前的记录置空
    currentLuckys = [];
    selectedCardIndex = [];
    ...
    if (leftCount < perCount) {
      addQipao(
        "Không có đủ số người tham gia xổ số, bây giờ đặt lại tất cả mọi người để có xổ số thứ hai!"
      );
    ...
    function (e) {
      if (music.paused) {
        music.play().then(
          ...
          () => {
            addQipao(
              "Nhạc nền không tự động phát được, vui lòng phát theo cách thủ công!"
            );
          }
        );
      } else {
